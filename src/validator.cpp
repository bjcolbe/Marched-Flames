...
string getError(cl_int);

void Validator::validateGPU(cl_int code, const char *file, int line) {
	if(code == CL_SUCCESS)
		return;

	fprintf(stderr, "%s:%i: %s\n", file, line, getError(code).c_str());
	exit(code);
}

string getError(cl_int code) {
	switch(code) {
		case 0:
			return "CL_SUCCESS";
		case -1:
